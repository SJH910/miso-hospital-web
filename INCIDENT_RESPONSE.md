...
*(2026-09-14 갱신)* "저는 X" 오탐 항목은 성씨 화이트리스트 검증으로 수정 완료됨 — 탐지→개선 루프가 이 사례에서도 끝까지 이어짐. spaCy 한국어 모델이 여전히 이 서버 환경엔 설치돼 있지 않아(`ko_core_news_sm` 미설치 경고가 실제로 뜸), 트리거 없이 등장하는 이름(NER 경로) 탐지는 아직 비활성 상태로 남아있다 — 별도 한계.

...
| "저는 X" 이름 탐지 정규식이 "실시간"/"미소병원"을 이름으로 오인식 | 문맥 구분 없는 패턴 | 아직 미수정(팀원 담당 영역으로 기록만) | `log_audit_tool.py` 재실행으로 실측 재현 |
...
마지막 항목("저는 X" 오탐)은 **아직 안 고쳐진 채로 남아있다** — 탐지는 잘 되는데 대응(코드 수정)이 아직 안 붙은 실제 사례라, 이 문서 3-4번("조치가 없는 경우")과 같은 패턴이 탐지 로직 자체에도 있다는 걸 보여준다.

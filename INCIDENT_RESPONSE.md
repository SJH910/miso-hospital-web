...
| **[신규 발견, 미수정] NER 모델을 이 서버에서 처음 켜자마자 새로운 오탐 다수 발생** — 위 수정과 같은 커밋이 `start.sh`도 고쳐서 `ko_core_news_sm` 미설치 시 자동 설치하게 만들었는데, 재시작 직후 이 컴퓨터에서 처음으로 NER이 실제로 켜짐 | sm 모델이 트리거 없는 일반 명사구까지 PERSON/PS로 오분류(정밀도 낮음) — "미소병원의 진료 시간은..."(트리거 없음)→`미***의`, "김치찌개"→`김***밥`. 여기에 `scan_for_pii()`가 `sender`(값: `bot`/`patient`) 같은 구조적 필드까지 구분 없이 `mask_pii()`에 통과시켜서 `bot`→`b*t`처럼 무관한 값도 오탐으로 잡힘 | **미수정** — 팀원이 준비 중인 것으로 보이는 `ko_core_news_md` 교체가 해결책일 가능성 있으나 어느 브랜치에도 아직 커밋 안 됨 | 재스캔 실측: `chatbot_sqlite` 발견 7→26건, `mysql_chat` 발견 2→63건으로 급증, 신규 오탐 사례(`미***의`/`김***밥`/`b*t`) 대시보드 "발견 내역"에서 직접 확인 |
...
*(2026-09-14 갱신)* "저는 X" 오탐 항목은 성씨 화이트리스트 검증으로 수정 완료됨 — 탐지→개선 루프가 이 사례에서도 끝까지 이어짐. 다만 같은 변경으로 `ko_core_news_sm`이 이 서버에서 처음 자동 설치·활성화되면서, 표 바로 위 행처럼 **새로운 종류의 오탐(NER발)이 즉시 관측됨** — "탐지 범위를 넓히면 새 오탐이 같이 따라온다"는 걸 실측으로 다시 확인한 사례.

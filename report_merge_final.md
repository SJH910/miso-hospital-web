...
- [ ] **진료과(`department`) 자유 입력 여부** — `reservations.department`는 지금 그냥 문자열(최대 50자)이라, 프론트나 챗봇을 통해 사용자가 "내과"/"내 과"(공백)/"Internal Medicine" 등 뭐든 입력한 그대로 저장됨 — 오타·표기 차이가 그대로 쌓일 수 있는 구조.
  ...
  - 아직 어느 쪽으로 갈지 결정 안 됨.
- [ ] **문의 답변 수정/이력 관리** — `answer` 컬럼 하나뿐이라 재답변 시 이전 답변이 덮어써지고 이력이 안 남음
- [ ] **감사 로그 보존 정책** — `audit_log`가 삭제/보관 기간 없이 무기한 적재됨
- [ ] **감사 로그 암호화 키 이원화 정리** — `secret.key`(챗봇 SQLite 감사 로그)와 `AUDIT_ENCRYPTION_KEY`(.env, audit-agent JSONL) 두 체계가 왜 따로 있는지 불명확. 통합하거나 역할을 문서화할 필요
- [ ] **파싱 실패/부정확 시 UX** — OCR 날짜·금액·기타 필드 파싱이 실패/오인식해도 관리자가 알아챌 방법이 없음
  - **환자는 실제로 볼 수 있음**: `records.html`의 "내 진료기록" 목록(`GET /api/documents/mine`)에 `parsed_date`/`parsed_amount`가 컬럼으로 표시되고, 상세보기(`GET /api/documents/mine/:id`)는 `extracted_text`(OCR 원문)를 그대로 화면에 렌더링함(`frontend/js/records.js`)
  - 환자가 원문과 파싱값을 눈으로 비교하면 오인식을 알아챌 수 있음
  - **정작 admin(입력한 사람)은 못 봄**: `admin.html` 목록(`GET /api/documents`)은 이름/날짜·시간/문서종류만 반환하고 파싱 결과·원문 전부 제외. `POST /api/documents` 저장 응답도 `{ id }`뿐이라 저장 직후에도 확인 불가 — **수정 권한이 있는 사람에게 확인 수단이 없는 게 진짜 문제**
  - **`parsed_fields`(범용 키-값)는 완전히 죽은 데이터**: API가 응답에 포함시키는데 `records.js`가 렌더링하는 코드 자체가 없어서 아무 화면에도 안 보임
  - 개선한다면: admin 목록/상세에 저장된 파싱 결과를 보여주는 게 우선순위가 더 높아 보임 (환자 쪽은 이미 어느 정도 가능)
